...
We have then two `Things`, available as IoT starter kits:
...
There are a couple ways to check `home-web-ir` with Internet browser. We can use the "Try it out!" button from Swagger IO and see detailed response fields, like message body, response code and headers, etc. Another simple way is to  address the service directly at the browser, using the `curl` command shown at "Request URL" field provided by Swagger IO.
