...
Current ASP.NET MVC `home-ui` is a suggestion, off course there are many other tools able to benefit from `home-web` API. But remember that since they both use same platform, `home-web` and `home-ui` share some image layers and  memory is optimized.
